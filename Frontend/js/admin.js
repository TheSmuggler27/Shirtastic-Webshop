document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    fetch("http://localhost/Shirtastic-Webshop/Backend/api/product_add.php", {
      method: "POST",
      body: formData
    })
    .then(res => res.json())
  ...
  function loadProducts() {
    fetch("http://localhost/Shirtastic-Webshop/Backend/api/products.php")
      .then(res => res.json())
      .then(data => {
        if (data.status === "ok") {
          ...
          data.products.forEach(product => {
            ...
            div.innerHTML = `
              <img src="/SHIRTASTIC-WEBSHOP/Frontend/img/${product.image_path}" alt="${product.name}" style="width: 100px;"><br>
          ...
          document.querySelectorAll(".delete-btn").forEach(button => {
            button.addEventListener("click", () => {
              ...
              if (confirm("Do you really want to delete this product?")) {
                fetch(`http://localhost/Shirtastic-Webshop/Backend/api/product_delete.php?id=${id}`, { method: "DELETE" })
                  .then(res => res.json())
                  .then(data => {
                    if (data.status === "ok") {
          ...
          document.querySelectorAll(".edit-btn").forEach(button => {
              ...
              fetch("http://localhost/Shirtastic-Webshop/Backend/api/product_update.php", {
                method: "POST",
